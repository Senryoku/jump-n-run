...
std::string md5FromText(const std::string &Text)
{
	uint32 len = (unsigned int) Text.size();
	...
	MD5Init(&context);
	MD5Update(&context, (const unsigned char *)Text.c_str(), len);
...
void md5RegressionTest(void)
{
	printf("md5 of haha: %s\n", md5FromText("haha").c_str());
